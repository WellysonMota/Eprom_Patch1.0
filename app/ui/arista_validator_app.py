"""
FTLC3351/3352R3PL1 — Arista Encoding Validator / Report  v1.0
EPS Global · Independent / batch validation of Arista-encoded transceivers
Same structural (power class / flextune / channel) checks as the Cisco validator,
identity section swapped for the Arista patch (Vendor Name, OUI, PN -A, SN -1,
cleared 0xE0-0xFB, CRC32 Arista Big-Endian).
"""
...
ARISTA_PN_SUFFIX   = "-A"
ARISTA_SN_SUFFIX   = "-1"
CLEAR_START, CLEAR_END = 0xE0, 0xFB          # 224-251, deve estar zerado (sem patch Cisco)
...
TARGET_B81    = 0xCF   # Power Class 7 (bits 7-6=11 + bits 1-0=11), CDR Tx/Rx presente
TARGET_B93    = 0x0D
LP_TARGETS    = {0x69: 0x01, 0x6A: 0x1F, 0x6B: 0x37, 0x71: 0x0E}   # 0x6B = Max Power 5.5W (Power Class 7 cap validado)
B0_TARGETS    = {0x80: 0x01, 0x81: 0x01}
TARGET_P1E_FD          = 0x02
TARGET_P1E_FLEXTUNE_EN   = 0x00   # ⚠️ Arista exige FlexTune DESLIGADO (diferente do Cisco, que era informacional)
...
# ─────────────────────────────────────────────────────────────────────────────
# ITU CHANNEL DICTIONARIES  (idêntico ao validador Cisco)
# ─────────────────────────────────────────────────────────────────────────────
CHAN_100GHZ = {
...
def resolve_channel(ch_raw, grid_spacing_code=0x5):
    ...
    if grid_spacing_code == 0x4:
        info = CHAN_50GHZ.get(ch_raw); label = "50GHz"
    else:
        info = CHAN_100GHZ.get(ch_raw); label = "100GHz"
    if info:
        return (info[0], info[1], info[2], label)
    return None

# ─────────────────────────────────────────────────────────────────────────────
# PARSER  (idêntico ao validador Cisco — mesmo formato Coherent IDE)
# ─────────────────────────────────────────────────────────────────────────────
...
def validate_dump(text: str, expected_channel: tuple = None) -> dict:
    ...
    vendor    = asc(upper,148,16).rstrip()
    oui_bytes = bytes([upper.get(165+i,0) for i in range(3)])
    date_code = asc(upper,212,8).strip()

    # ── Channel ──────────────────────────────────────────────────────────────
    cur_fgrid    = p1e.get(0xCB, 0x05) if p1e else 0x05
    grid_raw     = p12.get(0x80, 0x50) if p12 else 0x50
    grid_code    = (grid_raw >> 4) & 0x0F
    ...
    # ── Power class / structural checks (idêntico ao Cisco) ───────────────────
    checks.append(("Power Class (Upper 0x81) — Class 7", upper.get(129,0)==TARGET_B81,
    ...
    for addr, target in LP_TARGETS.items():
        cur = plow.get(addr, 0)
        label = f"Lower 0x{addr:02X}"
        if addr == 0x6B:
            label += " (Max Power — Power Class 7 cap, 5.5W)"
        checks.append((label, cur==target, f"0x{cur:02X}", f"0x{target:02X}"))
    if pb0:
    ...
    if p1e:
        ...
        # ⚠️ Diferente do Cisco: FlexTune precisa estar DESLIGADO pra Arista (check obrigatório)
        checks.append(("FlexTuneEnable ⚠️ MUST BE DISABLED (Arista)", cur_ften==TARGET_P1E_FLEXTUNE_EN,
                        f"0x{cur_ften:02X}", f"0x{TARGET_P1E_FLEXTUNE_EN:02X}"))
        checks.append(("ℹ️ FlexTuneGrid (informational, only relevant if FlexTune enabled)", True,
        ...
        checks.append(("FlexTuneEnable ⚠️ MUST BE DISABLED (Arista)", False,
                        "Page 1Eh absent", f"0x{TARGET_P1E_FLEXTUNE_EN:02X}"))
    ...
    # ── Arista patch (substitui a validação de patch Cisco) ────────────────────
    vendor_ok = vendor == ARISTA_VENDOR_NAME.decode().rstrip()
    checks.append(("Vendor Name", vendor_ok, vendor or "(empty)", "Arista Networks"))

    oui_ok = oui_bytes == ARISTA_OUI
    checks.append(("Vendor OUI", oui_ok, oui_bytes.hex().upper(), ARISTA_OUI.hex().upper()))

    pn_ok = pn.endswith(ARISTA_PN_SUFFIX)
    checks.append((f"Part Number ends with \"{ARISTA_PN_SUFFIX}\"", pn_ok, pn or "(empty)", f"...{ARISTA_PN_SUFFIX}"))

    sn_ok = sn.endswith(ARISTA_SN_SUFFIX)
    checks.append((f"Serial Number ends with \"{ARISTA_SN_SUFFIX}\"", sn_ok, sn or "(empty)", f"...{ARISTA_SN_SUFFIX}"))

    clear_bytes = bytes([upper.get(i,0xFF) for i in range(CLEAR_START, CLEAR_END+1)])
    clear_ok = all(b == 0 for b in clear_bytes)
    checks.append(("Bytes 0xE0-0xFB cleared (no Cisco patch residue)", clear_ok,
                    "all zero" if clear_ok else clear_bytes.hex().upper()[:20]+"...",
                    "all zero"))

    crc_stored = bytes([upper.get(CRC_OFF+i,0) for i in range(4)])
    crc_calc   = calc_arista_crc32(upper)
    crc_ok     = crc_stored == crc_calc
    checks.append(("CRC32 Arista (0xFC-0xFF, Big-Endian)", crc_ok,
                    crc_stored.hex().upper(), crc_calc.hex().upper()))

    # ── Page 02h (informational apenas — sem bloco fixo definido pra Arista) ───
    if p02:
        checks.append(("Page 02h present", True, "present", "informational only"))
    else:
        checks.append(("Page 02h present", False, "absent", "informational only"))

    all_ok = all(c[1] for c in checks)

    return {
        "error": None, "sn": sn, "pn": pn, "vendor": vendor, "oui": oui_bytes.hex().upper(),
        "date_code": date_code,
        ...
        "crc_arista": crc_stored.hex().upper(),
...
# UI
# ─────────────────────────────────────────────────────────────────────────────
st.set_page_config(page_title="Arista Encoding Validator", page_icon="✅", layout="wide")
st.markdown("""
<style>
...
.check-label{min-width:320px;font-weight:600;color:#2A2A2A}
...
with hb:
    st.markdown("""
    <p class="report-title">FTLC DCO Transceiver — Arista Encoding Validation Report
    <span style="font-size:13px;color:#999;font-weight:normal">v1.0</span></p>
    <p class="report-sub">EPS Global · Independent batch validation · SFF-8636 Rev 2.12 · Arista Compatibility</p>
    """, unsafe_allow_html=True)
st.markdown("---")
...
files = st.file_uploader(
    ...
    type=["txt"], accept_multiple_files=True, key="batch_upload_arista")
...
for r in results:
    ...
    with st.expander(exp_label, expanded=not r["all_ok"]):
        ...
        ft_lbl  = "Enabled ❌ (should be Disabled)" if r["flextune_en"]==1 else "Disabled ✅"
        ...
        summary_rows = [
            ("Vendor Name (Programmed)",       r["vendor"]),
            ("Vendor OUI (Programmed)",        r["oui"]),
            ("Part Number (Programmed)",       r["pn"]),
            ("Serial Number (Programmed)",     r["sn"]),
            ("Date Code",                      r["date_code"] if r["date_code"] else "—"),
            ("Channel",                        ch_disp),
            ("CRC32 Arista",                   f"<code style='font-size:12px;font-weight:bold'>{r['crc_arista']}</code>"),
...
csv_lines = ["filename,sn,pn,vendor,oui,date_code,channel_itu,freq_thz,wavelength_nm,crc_arista,result,failed_checks"]
for r in results:
    if r["error"]:
    ...
    csv_lines.append(
        f'{r["filename"]},{r["sn"]},{r["pn"]},{r["vendor"]},{r["oui"]},{r["date_code"]},'
        f'{ch_itu},{freq},{wl},{r["crc_arista"]},'
        f'{status},"{"; ".join(failed)}"')
csv_data = "\n".join(csv_lines)
st.download_button("📥 Download Validation Report (CSV)",
                   data=csv_data, file_name="Arista_Encoding_Validation_Report.csv",
                   mime="text/csv")
st.markdown("---")
st.caption("EPS Global · FTLC DCO Arista Encoding Validator v1.0 · "
           "Same power/flextune/channel checks as the Cisco validator, identity checks swapped for the Arista patch · SFF-8636 Rev 2.12")
