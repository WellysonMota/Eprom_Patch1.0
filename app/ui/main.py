...
def main():
    ...
    with col_k1:
        key_selection = st.selectbox(
            ...
            help="Selecione a chave de criptografia desejada para o patch"
        )
    ...
    if uploaded_file is not None:
        ...
        if len(file_bytes) < 256:
            ...
            print(MAGIC_KEYS)
            print(f"A seleção do cliente foi :" + key_selection)

            if key_selection in MAGIC_KEYS and key_selection != "JUNIPER" and key_selection != "INTEL":
                ...
                try:
                    ...
                    st.error(f"Erro no processamento 2: {e}")
                    st.info("Verifique se as funções no algorithms.py aceitam os argumentos de Rebrand.")

            elif key_selection == "JUNIPER":


                st.error(f"Erro no processamento: Função Juniper não implementada ")
                st.info("Verifique se as funções no algorithms.py aceitam os argumentos de Rebrand.")

            else:


                st.error(f"Erro no processamento: Função ainda não implementada 22 ")
                st.info("Verifique se as funções no algorithms.py aceitam os argumentos de Rebrand.")
